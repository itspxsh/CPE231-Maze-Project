...
public class GeneticSolverPureV7 implements MazeSolver {

    // HIGH SELECTION PRESSURE
    private static final int POPULATION_SIZE = 300;
    private static final int MAX_GENERATIONS = 600;
    private static final int TOURNAMENT_SIZE = 10; // Only the best of 10 get to reproduce
    ...
    public AlgorithmResult solve(MazeContext context) {
        ...
        for (int gen = 0; gen < MAX_GENERATIONS; gen++) {
            ...
            // Standard Elitism
            for (int i = 0; i < 5 && i < population.size(); i++) nextGen.add(population.get(i));

            while (nextGen.size() < POPULATION_SIZE) {
                // HIGH PRESSURE SELECTION
                Individual p1 = selectParentHighPressure(population);
                Individual p2 = selectParentHighPressure(population);
                
                Individual child;
                if (Math.random() < 0.9) child = twoPointCrossover(p1, p2, context);
                else child = p1;
        ...
        return new AlgorithmResult("GeneticPureV7", bestSolution.path, bestSolution.cost, System.nanoTime() - startTime, nodesExpanded);
    }

    // SPECIAL HELPER FOR V7
    private Individual selectParentHighPressure(List<Individual> pop) {
        Individual best = null;
        for (int i = 0; i < TOURNAMENT_SIZE; i++) { // Check 10 candidates
