...
public class GeneticSolverPureV2 implements MazeSolver {

    private static final int POPULATION_SIZE = 200;
    private static final int MAX_GENERATIONS = 500;
    private static final double CROSSOVER_RATE = 0.8;
    // Dynamic Mutation Variables
    ...
    public AlgorithmResult solve(MazeContext context) {
        ...
        for (int gen = 0; gen < MAX_GENERATIONS; gen++) {
            ...
            double currentMutationRate = START_MUTATION - ((START_MUTATION - END_MUTATION) * gen / MAX_GENERATIONS);

            List<Individual> nextGen = new ArrayList<>();
            // Keep top 5%
            int eliteCount = (int)(POPULATION_SIZE * 0.05);
            for (int i = 0; i < eliteCount; i++) nextGen.add(population.get(i));
            ...
            while (nextGen.size() < POPULATION_SIZE) {
                ...
                // Use dynamic rate
                if (Math.random() < currentMutationRate) child = mutate(child, context);
    ...
    // --- Helper Methods (Identical to V1 to ensure fairness, just Logic differs) ---
    // [COPY PASTE THE HELPER METHODS FROM V1 HERE: twoPointCrossover, initializePopulation, generateRandomValidPath, mutate, isValid, calculateCost, reconstructPath, selectParent, key]
    // To save space in this response, I assume you will copy the private methods from V1. 
    // They are completely standard.
    
    // ... (Paste private methods from V1 here) ...
