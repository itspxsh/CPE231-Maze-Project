...
public class GeneticSolverPureV8 implements MazeSolver {
    ...
    public AlgorithmResult solve(MazeContext context) {
        ...
        int stagnantGens = 0;
        double currentMutationRate = 0.5; // Start high
        ...
        for (int gen = 0; gen < MAX_GENERATIONS; gen++) {
            ...
            // ADAPTIVE LOGIC
            if (stagnantGens > STAGNATION_THRESHOLD) {
                currentMutationRate = 0.6; // Spike mutation to escape
            } else {
                // Decay mutation: 0.5 -> 0.05 over time
                currentMutationRate = 0.5 * (1.0 - ((double)gen / MAX_GENERATIONS));
                if (currentMutationRate < 0.05) currentMutationRate = 0.05;
            }
        ...
        return new AlgorithmResult("GeneticPureV8", bestSolution.path, bestSolution.cost, System.nanoTime() - startTime, nodesExpanded);
    }
