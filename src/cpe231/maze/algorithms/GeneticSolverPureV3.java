...
public class GeneticSolverPureV3 implements MazeSolver {
    ...
    private static final int STAGNATION_LIMIT = 25; // If no improvement for 25 gens...
    private static final double RESTART_PERCENTAGE = 0.7; // Replace 70% of population
    ...
    public AlgorithmResult solve(MazeContext context) {
        ...
        for (int gen = 0; gen < MAX_GENERATIONS; gen++) {
            ...
            if (stagnationCounter >= STAGNATION_LIMIT) {
                // System.out.println("Stagnation detected at Gen " + gen + ". Restarting population...");
                List<Individual> freshBlood = initializePopulation(context); // Generate new random paths
                int keepCount = (int)(POPULATION_SIZE * (1.0 - RESTART_PERCENTAGE));
                
                // Keep top 30% survivors, replace rest with fresh randoms
                for(int k=keepCount; k<POPULATION_SIZE; k++) {
                    if (k < freshBlood.size() + keepCount) {
                        population.set(k, freshBlood.get(k - keepCount));
                    }
                }
            ...
            for (int i = 0; i < 5; i++) nextGen.add(population.get(i)); // Elitism
        ...
        return new AlgorithmResult("Success", bestSolution.path, bestSolution.cost, System.nanoTime() - startTime, nodesExpanded);
    }

    // --- Helper Methods (Copy from V1/V2 for completeness) ---
    // [COPY PASTE THE HELPER METHODS FROM V1 HERE AGAIN]
    // To ensure the file is complete for you, I'm pasting them below.
