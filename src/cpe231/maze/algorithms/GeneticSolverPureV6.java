...
public class GeneticSolverPureV6 implements MazeSolver {

    // CATACLYSM PARAMETERS
    private static final int POPULATION_SIZE = 200;
    private static final int MAX_GENERATIONS = 800;
    private static final int STAGNATION_LIMIT = 50; // Trigger reset if stuck for 50 gens
    ...
    public AlgorithmResult solve(MazeContext context) {
        ...
        for (int gen = 0; gen < MAX_GENERATIONS; gen++) {
            ...
            // --- THE CATACLYSM ---
            if (stagnantGens >= STAGNATION_LIMIT) {
                List<Individual> survivors = new ArrayList<>();
                survivors.add(bestSolution); // Save the king
                // Wipe the rest and restart with fresh random paths
                List<Individual> freshBlood = initializePopulation(context);
                survivors.addAll(freshBlood);
                // Trim to size
                while(survivors.size() > POPULATION_SIZE) survivors.remove(survivors.size()-1);
                population = survivors;
                stagnantGens = 0; // Reset counter
            } else {
                // Normal Evolution
                List<Individual> nextGen = new ArrayList<>();
                nextGen.add(population.get(0)); // Elite 1
                while (nextGen.size() < POPULATION_SIZE) {
                    Individual p1 = selectParent(population);
                    Individual p2 = selectParent(population);
                    Individual child;
                    if (Math.random() < 0.8) child = twoPointCrossover(p1, p2, context);
                    else child = p1;
                    
                    if (Math.random() < 0.2) child = mutate(child, context);
                    nextGen.add(child);
                    nodesExpanded++;
                }
                population = nextGen;
        ...
        return new AlgorithmResult("GeneticPureV6", bestSolution.path, bestSolution.cost, System.nanoTime() - startTime, nodesExpanded);
    }
